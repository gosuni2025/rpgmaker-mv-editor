...
게임 내 UI 창에서 자주 사용되는 테두리(프레임)를 시각적으로 편집합니다. windowskin 이미지에서 배경과 테두리 영역을 지정하고, 9-slice 방식으로 어떤 크기의 창에도 자연스럽게 늘어나도록 설정합니다.

![9-slice 스킨 에디터](images/ui-editor/skin-editor.png)

- **좌측**: 등록된 스킨 목록 — 게이지(HP/MP/TP), 커스텀 스킨 등을 각각 관리
- **중앙 캔버스**: windowskin 이미지 위에서 프레임 영역과 배경 영역을 드래그로 직접 지정
- **우측 인스펙터**: 프레임 영역(X/Y/너비/높이), Fill 영역(배경), 코너 크기를 수치로 조정
- **프리뷰**: 실제 적용 모습을 실시간으로 미리 확인
...
![샘플 타이틀 씬 편집](images/ui-editor/ui-title-navigation.png)

에디터에는 타이틀, 옵션, 스테이터스 등 자주 사용하는 씬이 샘플로 포함되어 있습니다. 처음부터 만들 필요 없이, 샘플 씬을 선택한 뒤 버튼 위치·텍스트·배경 등을 수정하는 것만으로 간편하게 나만의 UI를 완성할 수 있습니다.

**Nav Visual** 모드를 켜면 버튼 간의 네비게이션 연결을 시각적으로 확인·편집할 수 있습니다. 파란색/주황색 선이 방향키 입력 시 포커스가 이동하는 경로를 나타냅니다.
