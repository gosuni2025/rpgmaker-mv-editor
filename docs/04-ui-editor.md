...
```
[상 편집] [프레임 편집] [커서 편집] [폰트 설정]    [저장] [현재 맵에서 테스트]
─────────────────────────────────────────────────────────────────────────────
씬 선택:  [옵션 (Scene_Options) ▼]  [+]    씬: Scene_Options    [새로고침] [등장] [퇴장]
──────────────────────────────────────────────────────────────────────────
 씬 목록              │  캔버스 (씬 프리뷰)               │  씬 설정 / 인스펙터
 ──────────────────  │  ──────────────────────────────  │  ──────────────────
 ● Options            │                                  │  SCENE_OPTIONS
   Scene_Options      │    ┌──────────────────────────┐ │  씬 교체
   Window_Options     │    │  항상 대쉬      OFF       │ │  ○ 인 게임 기본 씬 사용
                      │    │  명령 기억      OFF       │ │  ● 커스텀 씬으로 교체
                      │    │  마우스 커서 표시ON        │ │
                      │    │  BGM 볼륨      100%      │ │  이 씬의 기본 폰트 설정
                      │    │  BGS 볼륨      100%      │ │  현재: GameFont
                      │    └──────────────────────────┘ │  [폰트 변경...]
```
...
```
┌──┬──────────────────┬──┐
│TL│     Top Edge     │TR│  ← cornerSize 크기의 모서리
├──┼──────────────────┼──┤
│  │                  │  │
│Le│   fill 영역      │Ri│  ← fillX/Y/W/H 으로 정의
│  │                  │  │
├──┼──────────────────┼──┤
│BL│   Bottom Edge    │BR│
└──┴──────────────────┴──┘
