...
맵 캔버스에서 목적지와 경유지를 직접 클릭해 경로를 시각적으로 설정합니다. 빨간 점(웨이포인트)과 점선이 이동 경로를 나타내며, A* 알고리즘이 장애물을 자동으로 회피하는 최단 경로를 계산합니다.

- **편집 시작** 버튼을 누르면 캔버스가 웨이포인트 편집 모드로 전환됩니다.
- 맵 위를 클릭하면 웨이포인트가 추가되고, 경유지 간 경로가 자동으로 연결됩니다.
- 우측 인스펙터의 **확정** 버튼을 누르면 경로가 이동 루트 커맨드로 변환·저장됩니다.
