...
### 디렉토리 기반 다중 파일 플러그인 (Directory Plugin)

거대한 단일 파일 플러그인을 여러 파일로 분할하여 개발성을 높이는 구조. **서버가 투명하게 번들링**하므로 런타임(RPG Maker MV PluginManager)은 단일 파일과 동일하게 인식함.

```
server/runtime/js/plugins/
  MyPlugin.js          # 기존 단일 파일 플러그인 (변경 없음)
  CustomSceneEngine/   # 디렉토리 플러그인
    _plugin.js         # 메타데이터 + IIFE 열기 + 공유 변수 선언
    01_DataLoaders.js  # 숫자 접두사 순서대로 로드
    02_Templates.js
    ...
    _end.js            # IIFE 닫기: })();
```
...
**규칙**:
- `_plugin.js` 존재 = 디렉토리 플러그인으로 인식
- 로딩 순서: `_plugin.js` → 숫자 접두사 순 → 접두사 없는 파일 알파벳순 → `_end.js`
- IIFE 클로저 공유: 파일들이 동일 IIFE 스코프에서 연결되므로 변수 공유 자동
- 마이그레이션 시 번들→ 단일 `.js`로 프로젝트에 복사됨
- 번들링 로직: `server/services/pluginBundler.ts`
...
- **방법**: 수정한 파일을 프로젝트 폴더의 동일 상대 경로에 복사 (디렉토리 플러그인은 번들 후 복사)
