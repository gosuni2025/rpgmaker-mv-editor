...
export function MovePictureEditor({ p, onOk, onCancel }: { p: unknown[]; onOk: (params: unknown[]) => void; onCancel: () => void }) {
  ...
  return (
    <>
      ...
      <div style={{ display: 'flex', gap: 8 }}>
        <Fieldset legend="위치" style={{ flex: 1 }}>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
            ...
            <DirectPositionInputs posX={positionType === 0 ? posX : 0} posY={positionType === 0 ? posY : 0}
              onPosXChange={setPosX} onPosYChange={setPosY} disabled={positionType !== 0} />

            <label style={radioStyle}>
              <input type="radio" name="movepic-pos-type" checked={positionType === 1} onChange={() => setPositionType(1)} />
              변수로 지정
            </label>
            <VariablePositionInputs posX={positionType === 1 ? (posX || 1) : 1} posY={positionType === 1 ? (posY || 1) : 1}
              onPosXChange={setPosX} onPosYChange={setPosY} disabled={positionType !== 1} />
          </div>
        </Fieldset>
      ...
      <EditorFooter onCancel={onCancel} onOk={() =>
        onOk([pictureNumber, '', origin, positionType, posX, posY, scaleWidth, scaleHeight, opacity, blendMode, duration, waitForCompletion])
      } />
    </>
  );
}
