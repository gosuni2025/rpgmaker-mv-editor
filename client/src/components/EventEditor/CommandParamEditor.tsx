...
import {
  ...
  ScrollingTextEditor, ConditionalBranchEditor, ShowPictureEditor, RotatePictureEditor,
} from './commandEditors';
...
export default function CommandParamEditor({ code, command, followCommands, hasElse, onOk, onCancel }: CommandParamEditorProps) {
  ...
  const dialogWidth = code === 102 ? 560 : code === 111 ? 540 : code === 231 ? 520 : 480;
  return (
...
function getCommandName(code: number): string {
  const names: Record<number, string> = {
    ...
    201: '장소 이동', 202: '탈 것 위치 설정', 204: '지도 스크롤', 211: '투명 상태 변경', 212: '애니메이션 표시', 213: '말풍선 아이콘 표시', 216: '대열 보행 변경', 230: 'Wait', 231: '그림 표시', 233: '그림 회전', 235: '그림 제거',
