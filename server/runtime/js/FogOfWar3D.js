...
var BOX_FOG_VERT = [
    ...
    'uniform sampler2D tFog;',         // 가시성 텍스처 (vertex에서도 참조)
    '',
    'void main() {',
    ...
    '',
    '    // fog 텍스처에서 가시성 샘플링 → 높이 스케일링',
    '    vec4 fogSample = texture2D(tFog, vTileUV);',
    '    float visibility = fogSample.r;',
    '    vVisibility = visibility;',
    '',
    '    // visibility에 따라 박스 높이를 축소 (가시=높이0, 미탐험=원래높이)',
    '    float heightScale = 1.0 - visibility;',
    '',
    '    vLocalPos = position;',
    '    // position.z: -0.5 ~ 0.5 (BoxGeometry 기준)',
    '    // 높이 스케일 적용: 바닥(z=-0.5)은 고정, 윗면만 내려감',
    '    vec3 scaledPos = position;',
    '    scaledPos.z = -0.5 + (position.z + 0.5) * heightScale;',
    '    // 정규화된 높이: 0(바닥)~1(꼭대기), 스케일 후에도 0~1 유지',
    '    vHeightNorm = heightScale > 0.001 ? (scaledPos.z + 0.5) / heightScale : 0.0;',
    '',
...
var BOX_FOG_FRAG = [
    ...
    '',
    '    // 2. 완전 가시 타일은 discard (vertex에서 높이=0으로 이미 축소됨)',
    '    if (vVisibility > 0.99) discard;',
    '',
    '    // 3. 기본 알파 계산',
    '    float baseAlpha;',
    '    if (explored < 0.5) {',
