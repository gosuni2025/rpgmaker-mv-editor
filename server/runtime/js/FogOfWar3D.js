...
var BOX_FOG_FRAG = [
    ...
    '}',
    '',
    '// 경계 판정: 미탐험 타일이면서 인접에 탐험이 있으면 경계',
    'bool isBorderTile(vec2 uv) {',
    '    if (sampleExplNearest(uv) > 0.5) return false;',
    '    vec2 texel = 1.0 / mapSize;',
    '    return sampleExplNearest(uv + vec2(-texel.x, 0.0)) > 0.5',
    '        || sampleExplNearest(uv + vec2( texel.x, 0.0)) > 0.5',
    '        || sampleExplNearest(uv + vec2(0.0, -texel.y)) > 0.5',
    '        || sampleExplNearest(uv + vec2(0.0,  texel.y)) > 0.5',
    '        || sampleExplNearest(uv + vec2(-texel.x, -texel.y)) > 0.5',
    '        || sampleExplNearest(uv + vec2( texel.x, -texel.y)) > 0.5',
    '        || sampleExplNearest(uv + vec2(-texel.x,  texel.y)) > 0.5',
    '        || sampleExplNearest(uv + vec2( texel.x,  texel.y)) > 0.5;',
    '}',
    '',
    '// 탐험 경계 판정: 탐험 타일이면서 인접에 미탐험이 있으면 경계',
    'bool isExplBorderTile(vec2 uv) {',
    '    if (sampleExplNearest(uv) < 0.5) return false;',
    '    vec2 texel = 1.0 / mapSize;',
    '    return sampleExplNearest(uv + vec2(-texel.x, 0.0)) < 0.5',
    '        || sampleExplNearest(uv + vec2( texel.x, 0.0)) < 0.5',
    ...
    '}',
    '',
    '// 경계의 fade 값: 미탐험→growFade, 탐험→tentacleFade',
    'float borderFade(vec2 uv) {',
    '    if (sampleExplNearest(uv) < 0.5) return sampleGrowNearest(uv);',
    '    return sampleFadeNearest(uv);',
    '}',
    '',
    '// 3-layer 노이즈 → 촉수 길이 계산',
    'float calcTentacleNoise(vec2 noiseInput, float timeS, out float nNorm) {',
    '    float n1 = fbm3(noiseInput * 0.025 + vec2(timeS * 0.06, timeS * 0.04));',
    '    float n2 = _valueNoise(noiseInput * 0.07 + vec2(-timeS * 0.08, timeS * 0.05));',
    '    float n3 = _valueNoise(noiseInput * 0.15 + vec2(timeS * 0.12, -timeS * 0.07));',
    '    float noise = n1 * 0.5 + n2 * 0.3 + n3 * 0.2;',
    '    nNorm = clamp(noise + 0.5, 0.0, 1.0);',
    '    return pow(nNorm, tentacleSharpness) * dissolveStrength;',
    '}',
    '',
    '// 가장 가까운 탐험 타일 경계까지의 거리 탐색 (미탐험 타일용)',
    '// 탐험된 이웃이나 탐험 경계 타일까지의 거리와 borderFade를 반환',
    'void searchExplBorder(vec2 uv, vec2 tp, out float dist, out float bf, out bool found) {',
    '    vec2 texel = 1.0 / mapSize;',
    '    dist = 99.0; bf = 1.0; found = false;',
    '    float diag = 1.414;',
    ...
    '        if (r == 0.0) {',
    '            // 자기 자신은 미탐험 경계: 인접 탐험 방향까지의 타일 내 거리',
    '            if (sampleExplNearest(uv + vec2(-texel.x, 0.0)) > 0.5) { float d = tp.x;           float b = sampleGrowNearest(uv); if (d < dist) { dist = d; bf = b; } found = true; }',
    '            if (sampleExplNearest(uv + vec2( texel.x, 0.0)) > 0.5) { float d = 1.0 - tp.x;    float b = sampleGrowNearest(uv); if (d < dist) { dist = d; bf = b; } found = true; }',
    '            if (sampleExplNearest(uv + vec2(0.0, -texel.y)) > 0.5) { float d = tp.y;           float b = sampleGrowNearest(uv); if (d < dist) { dist = d; bf = b; } found = true; }',
    '            if (sampleExplNearest(uv + vec2(0.0,  texel.y)) > 0.5) { float d = 1.0 - tp.y;    float b = sampleGrowNearest(uv); if (d < dist) { dist = d; bf = b; } found = true; }',
    '            if (sampleExplNearest(uv + vec2(-texel.x, -texel.y)) > 0.5) { float d = length(tp);                              float b = sampleGrowNearest(uv); if (d < dist) { dist = d; bf = b; } found = true; }',
    '            if (sampleExplNearest(uv + vec2( texel.x, -texel.y)) > 0.5) { float d = length(vec2(1.0-tp.x, tp.y));           float b = sampleGrowNearest(uv); if (d < dist) { dist = d; bf = b; } found = true; }',
    '            if (sampleExplNearest(uv + vec2(-texel.x,  texel.y)) > 0.5) { float d = length(vec2(tp.x, 1.0-tp.y));           float b = sampleGrowNearest(uv); if (d < dist) { dist = d; bf = b; } found = true; }',
    '            if (sampleExplNearest(uv + vec2( texel.x,  texel.y)) > 0.5) { float d = length(vec2(1.0-tp.x, 1.0-tp.y));      float b = sampleGrowNearest(uv); if (d < dist) { dist = d; bf = b; } found = true; }',
    '        } else {',
    '            // r>=1: 탐험 경계 또는 탐험된 타일을 찾음',
    '            if (isExplBorderTile(uvL) || sampleExplNearest(uvL) > 0.5) { float d = (r - 1.0) + tp.x;       float b = borderFade(uvL);  if (d < dist) { dist = d; bf = b; } found = true; }',
    '            if (isExplBorderTile(uvR) || sampleExplNearest(uvR) > 0.5) { float d = (r - 1.0) + (1.0-tp.x); float b = borderFade(uvR);  if (d < dist) { dist = d; bf = b; } found = true; }',
    '            if (isExplBorderTile(uvU) || sampleExplNearest(uvU) > 0.5) { float d = (r - 1.0) + tp.y;       float b = borderFade(uvU);  if (d < dist) { dist = d; bf = b; } found = true; }',
    '            if (isExplBorderTile(uvD) || sampleExplNearest(uvD) > 0.5) { float d = (r - 1.0) + (1.0-tp.y); float b = borderFade(uvD);  if (d < dist) { dist = d; bf = b; } found = true; }',
    ...
    '            if (isExplBorderTile(uvLU) || sampleExplNearest(uvLU) > 0.5) { float d = (r - 1.0) * diag + length(tp);                              float b = borderFade(uvLU); if (d < dist) { dist = d; bf = b; } found = true; }',
    '            if (isExplBorderTile(uvRU) || sampleExplNearest(uvRU) > 0.5) { float d = (r - 1.0) * diag + length(vec2(1.0-tp.x, tp.y));           float b = borderFade(uvRU); if (d < dist) { dist = d; bf = b; } found = true; }',
    '            if (isExplBorderTile(uvLD) || sampleExplNearest(uvLD) > 0.5) { float d = (r - 1.0) * diag + length(vec2(tp.x, 1.0-tp.y));           float b = borderFade(uvLD); if (d < dist) { dist = d; bf = b; } found = true; }',
    '            if (isExplBorderTile(uvRD) || sampleExplNearest(uvRD) > 0.5) { float d = (r - 1.0) * diag + length(vec2(1.0-tp.x, 1.0-tp.y));      float b = borderFade(uvRD); if (d < dist) { dist = d; bf = b; } found = true; }',
    '        }',
    '    }',
    '}',
    '',
    '// 가장 가까운 미탐험 경계까지의 거리 탐색 (탐험된 타일용, 2D FOW와 동일)',
    'void searchUnexplBorder(vec2 uv, vec2 tp, out float dist, out float bf, out bool found) {',
    '    vec2 texel = 1.0 / mapSize;',
    '    dist = 99.0; bf = 1.0; found = false;',
    '    float diag = 1.414;',
    ...
    '        vec2 uvU  = uv + vec2(0.0, -texel.y * r);',
    '        vec2 uvD  = uv + vec2(0.0,  texel.y * r);',
    '        if (isBorderTile(uvL))  { float d = (r - 1.0) + tp.x;       float b = borderFade(uvL);  if (d < dist) { dist = d; bf = b; } found = true; }',
    '        if (isBorderTile(uvR))  { float d = (r - 1.0) + (1.0-tp.x); float b = borderFade(uvR);  if (d < dist) { dist = d; bf = b; } found = true; }',
    '        if (isBorderTile(uvU))  { float d = (r - 1.0) + tp.y;       float b = borderFade(uvU);  if (d < dist) { dist = d; bf = b; } found = true; }',
    '        if (isBorderTile(uvD))  { float d = (r - 1.0) + (1.0-tp.y); float b = borderFade(uvD);  if (d < dist) { dist = d; bf = b; } found = true; }',
    '        vec2 uvLU = uv + vec2(-texel.x * r, -texel.y * r);',
    '        vec2 uvRU = uv + vec2( texel.x * r, -texel.y * r);',
    '        vec2 uvLD = uv + vec2(-texel.x * r,  texel.y * r);',
    '        vec2 uvRD = uv + vec2( texel.x * r,  texel.y * r);',
    '        if (isBorderTile(uvLU)) { float d = (r - 1.0) * diag + length(tp);                              float b = borderFade(uvLU); if (d < dist) { dist = d; bf = b; } found = true; }',
    '        if (isBorderTile(uvRU)) { float d = (r - 1.0) * diag + length(vec2(1.0-tp.x, tp.y));           float b = borderFade(uvRU); if (d < dist) { dist = d; bf = b; } found = true; }',
    '        if (isBorderTile(uvLD)) { float d = (r - 1.0) * diag + length(vec2(tp.x, 1.0-tp.y));           float b = borderFade(uvLD); if (d < dist) { dist = d; bf = b; } found = true; }',
    '        if (isBorderTile(uvRD)) { float d = (r - 1.0) * diag + length(vec2(1.0-tp.x, 1.0-tp.y));      float b = borderFade(uvRD); if (d < dist) { dist = d; bf = b; } found = true; }',
    '    }',
    '}',
    '',
    'void main() {',
    ...
    '    vec2 tp = fract(vTileUV * mapSize);',
    '',
    '    // === 미탐험 타일 ===',
    '    if (myExpl < 0.5) {',
    '        if (edgeAnimOn < 0.5) {',
    '            // 에지 애니메이션 꺼짐: heightFalloff만 적용',
    '            if (isTopFace || isSideFace) {',
    ...
    '        }',
    '',
    '        // 탐험 경계까지의 거리 탐색 (미탐험 → 탐험 방향)',
    '        float distToExpl; float explBf; bool foundExpl;',
    '        searchExplBorder(vTileUV, tp, distToExpl, explBf, foundExpl);',
    '',
    '        if (!foundExpl) {',
    ...
    '        // 노이즈 → 촉수 길이',
    '        float nNorm;',
    '        float fullLength = calcTentacleNoise(worldXY, timeS, nNorm);',
    '        float tentacleLength = fullLength * explBf;',
    '',
    '        if (isTopFace) {',
    '            // 윗면: 탐험 경계에서 바깥으로 침투하는 fog 제거(discard)',
    '            if (distToExpl < tentacleLength) discard;',
    '        } else if (isSideFace) {',
    '            // 옆면: 높이에 따른 노이즈 디졸브',
    '            vec2 sideNoise = worldXY + vec2(vWorldZ * 0.7);',
    '            float sideNNorm;',
    '            float sideLen = calcTentacleNoise(sideNoise, timeS, sideNNorm);',
    '            sideLen *= explBf;',
    '            // 높이가 높을수록 더 많이 침식',
    '            float heightBias = heightNorm * 0.6;',
    '            if (distToExpl + heightBias < sideLen) discard;',
    '        } else {',
    '            // 바닥면: 항상 그리기',
    '        }',
    '',
    ...
    '',
    '    // === 탐험된 타일 ===',
    '    // 2D FOW 방식: 미탐험 경계까지의 거리 탐색 + 촉수 침투',
    '    if (edgeAnimOn < 0.5) discard;',
    '',
    '    float distToBorder; float closestBf; bool foundBorder;',
    '    searchUnexplBorder(vTileUV, tp, distToBorder, closestBf, foundBorder);',
    '',
    ...
    '    float nNorm;',
    '    float fullLength = calcTentacleNoise(worldXY, timeS, nNorm);',
    '    float tentacleLength = fullLength * closestBf;',
    '    float inTentacle = tentacleLength - distToBorder;',
    '',
    '    if (isTopFace) {',
    '        // 윗면: 2D FOW와 동일한 촉수 침투 표현',
    '        if (inTentacle > 0.0) {',
    '            float alpha = smoothstep(0.0, fadeSmoothness, inTentacle);',
    ...
    '        }',
    '    } else if (isSideFace) {',
    '        // 옆면: 높이에 따라 감쇠하는 촉수 침투',
    '        float heightAtten = 1.0 - heightNorm * 0.7;',
    '        float sideInTentacle = tentacleLength * heightAtten - distToBorder;',
    '        if (sideInTentacle > 0.0) {',
    '            float alpha = smoothstep(0.0, fadeSmoothness, sideInTentacle);',
    '            alpha *= smoothstep(0.0, 0.25, nNorm);',
    '            alpha *= unexploredAlpha * (1.0 - heightNorm * 0.5);',
    '            if (alpha < 0.001) discard;',
    '            gl_FragColor = vec4(color, alpha);',
    '            return;',
    '        }',
...
var TENTACLE_VERT = [
    ...
    '',
    '    // 빌보드 리본: 뷰 방향과 촉수 접선의 cross product로 측면 벡터 계산',
    '    vec3 viewDir = normalize(cameraPosition - pos);',
    '    vec3 tangent = normalize(outDir * effectiveLen + (perp1 * off1 + perp2 * off2) * lenScale);',
    '    vec3 billboardDir = cross(tangent, viewDir);',
    '    float bLen = length(billboardDir);',
    '    // 카메라가 접선과 평행할 때 fallback',
    '    billboardDir = bLen > 0.01 ? billboardDir / bLen : perp1;',
