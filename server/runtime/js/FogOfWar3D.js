...
var BOX_FOG_FRAG = [
    ...
    '    return sampleExplNearest(uv) < 0.5;',
    '}',
    ...
    '',
    '    // 3. 현재 프래그먼트의 정규화 높이 (0=바닥, 1=꼭대기)',
    '    float heightNorm = clamp(vWorldZ / fogHeight, 0.0, 1.0);',
    '',
    '    vec2 texel = 1.0 / mapSize;',
    '    float myExpl = sampleExplNearest(vTileUV);',
    '',
    '    // 월드 XY 기반 노이즈 (2D FOW와 동일한 3-레이어 노이즈)',
    '    vec2 worldXY = vWorldPos.xy + scrollOffset;',
    '    float timeS = uTime * edgeAnimSpeed;',
    '    float n1 = fbm3(worldXY * 0.025 + vec2(timeS * 0.06, timeS * 0.04));',
    '    float n2 = _valueNoise(worldXY * 0.07 + vec2(-timeS * 0.08, timeS * 0.05));',
    '    float n3 = _valueNoise(worldXY * 0.15 + vec2(timeS * 0.12, -timeS * 0.07));',
    '    float noise = n1 * 0.5 + n2 * 0.3 + n3 * 0.2;',
    '    float nNorm = clamp(noise + 0.5, 0.0, 1.0);',
    '',
    '    // 촉수 높이 (노이즈 기반, 정규화: 0~1)',
    '    float tentacleNorm = pow(nNorm, tentacleSharpness) * dissolveStrength * tileSize / fogHeight;',
    '',
    '    if (myExpl < 0.5) {',
    '        // === 미탐험 타일 ===',
    ...
    '            // 경계 미탐험 타일:',
    '            // 촉수가 꼭대기에서 돌출 — 노이즈로 윗라인이 들쭉날쭉',
    '            float gf = sampleGrowNearest(vTileUV);',
    '            float tH = tentacleNorm * gf;',
    '            // cutoff 아래 = 불투명 fog 본체, cutoff 위 = 촉수 실루엣 (discard)',
    '            float cutoff = 1.0 - tH;',
    ...
    '',
    '        if (border && edgeAnimOn > 0.5) {',
    '            // 경계 탐험 타일: tentacleFade로 촉수가 바닥에서 솟아오름',
    '            float tf = sampleFadeNearest(vTileUV);',
    '            float tH = tentacleNorm * tf;',
    '            if (tH > 0.001 && heightNorm < tH) {',
    '                // 촉수 영역: 불투명으로 그림',
    '                vec3 color = heightGradientOn > 0.5 ? mix(fogColor, fogColorTop, heightNorm) : fogColor;',
    '                gl_FragColor = vec4(color, unexploredAlpha);',
    '                return;',
