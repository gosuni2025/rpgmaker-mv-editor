...
var VOL_FOG_FRAG = [
    ...
    '}',
    'float sampleGrowNearestFog(vec2 uv, vec2 texel) {',
    '    vec2 snapped = (floor(uv / texel) + 0.5) * texel;',
    '    return texture2D(tFog, snapped).a;',
    '}',
    '// 경계 판정: grow가 충분히 진행된 미탐험이거나 fade 중인 타일',
    'bool isBorderFog(vec2 uv, vec2 texel) {',
    '    if (sampleFadeNearestFog(uv, texel) > 0.01) return true;',
    '    if (sampleExplNearestFog(uv, texel) < 0.5 && sampleGrowNearestFog(uv, texel) > 0.5) return true;',
    '    return false;',
    '}',
    '',
...
var EDGE_DISSOLVE_FRAG = [
    ...
    '    if (sampleFadeNearest(uv, texel) > 0.01) return true;',
    '    if (sampleExplNearest(uv, texel) < 0.5 && sampleGrowNearest(uv, texel) > 0.5) return true;',
    '    return false;',
    '}',
    '// 경계 타일의 fade값',
    ...
    '        // closestBf: 촉수의 생성/사라짐 fade (촉수에만 적용)',
    '        alpha *= closestBf;',
    '    } else {',
    '        // 촉수 바깥: 정상 fog (fade 무관)',
    '        alpha = exploredAlpha;',
    '    }',
    '',
