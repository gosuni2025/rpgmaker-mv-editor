...
var EDGE_DISSOLVE_FRAG = [
    ...
    '        if (eLU < 0.5) minD = min(minD, length(tp));',
    '        if (eRU < 0.5) minD = min(minD, length(vec2(1.0 - tp.x, tp.y)));',
    '        if (eLD < 0.5) minD = min(minD, length(vec2(tp.x, 1.0 - tp.y)));',
    '        if (eRD < 0.5) minD = min(minD, length(1.0 - tp));',
    '        signedDist = minD;',
    '    } else {',
