...
var VOL_FOG_FRAG = [
    ...
    '// 시야 안: 완전 투명',
    '// 미탐험/탐험 어두운 곳: 불투명',
    '// 경계면(미탐험의 안쪽 가장자리)에만: 알파 그라데이션 + 디졸브',
    'void render2D() {',
    ...
    '',
    '    // --- 경계면 판정 ---',
    '    // nearVis(B채널) > 0 = 시야 근처 (확산된 가시성)',
    '    // visibility = 0이지만 nearVis > 0인 곳 = 미탐험/탐험의 안쪽 가장자리',
    '    float edgeDist = nearVis * nearVisWeight;',
    '',
    '    // --- 디졸브 노이즈 (경계면에서만 작동) ---',
    ...
    '',
    '    // 디졸브로 경계 위치를 불규칙하게 변형',
    '    edgeDist += dn * dissolveStrength;',
    '    edgeDist = max(edgeDist, 0.0);',
    '',
    '    // --- fog alpha ---',
    '    float fogAlpha;',
    '    if (edgeDist > 0.005) {',
    '        // 경계면: edgeDist가 클수록 시야에 가까움 → 투명',
    '        float fade = smoothstep(0.0, fadeSmoothness, edgeDist);',
