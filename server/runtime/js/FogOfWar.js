...
var EDGE_DISSOLVE_FRAG = [
    ...
    '',
    '    // 경계 탐색: 각 방향의 경계까지 유효 거리 계산',
    '    // 실제 미탐험: 거리 그대로',
    '    // fade 중인 타일: fade 값으로 거리를 밀어냄 (fade→0이면 거리→∞)',
    '    // → 각 방향이 독립적으로 fade되므로 방향별 촉수 소멸 가능',
    ...
    '        // 4방향',
    '        if (isBorder(uvL, texel)) { float d = (r - 1.0) + tp.x; float bf = borderFade(uvL, texel); d /= max(bf, 0.01); if (d < distToBorder) distToBorder = d; foundBorder = true; }',
    '        if (isBorder(uvR, texel)) { float d = (r - 1.0) + (1.0 - tp.x); float bf = borderFade(uvR, texel); d /= max(bf, 0.01); if (d < distToBorder) distToBorder = d; foundBorder = true; }',
    '        if (isBorder(uvU, texel)) { float d = (r - 1.0) + tp.y; float bf = borderFade(uvU, texel); d /= max(bf, 0.01); if (d < distToBorder) distToBorder = d; foundBorder = true; }',
    '        if (isBorder(uvD, texel)) { float d = (r - 1.0) + (1.0 - tp.y); float bf = borderFade(uvD, texel); d /= max(bf, 0.01); if (d < distToBorder) distToBorder = d; foundBorder = true; }',
    '        // 대각선',
    ...
    '        if (isBorder(uvLU, texel)) { float d = (r - 1.0) * diag + length(tp); float bf = borderFade(uvLU, texel); d /= max(bf, 0.01); if (d < distToBorder) distToBorder = d; foundBorder = true; }',
    '        if (isBorder(uvRU, texel)) { float d = (r - 1.0) * diag + length(vec2(1.0 - tp.x, tp.y)); float bf = borderFade(uvRU, texel); d /= max(bf, 0.01); if (d < distToBorder) distToBorder = d; foundBorder = true; }',
    '        if (isBorder(uvLD, texel)) { float d = (r - 1.0) * diag + length(vec2(tp.x, 1.0 - tp.y)); float bf = borderFade(uvLD, texel); d /= max(bf, 0.01); if (d < distToBorder) distToBorder = d; foundBorder = true; }',
    '        if (isBorder(uvRD, texel)) { float d = (r - 1.0) * diag + length(vec2(1.0 - tp.x, 1.0 - tp.y)); float bf = borderFade(uvRD, texel); d /= max(bf, 0.01); if (d < distToBorder) distToBorder = d; foundBorder = true; }',
    '    }',
    '',
    ...
    '',
    '    // myFade: bilinear 보간된 fade 텍스처 (경계 근처=높음, 중심=0)',
    '    // fade 중 가장자리→중심으로 점점 투명해지는 그라데이션',
    '    float fadeAlpha = (myFade > 0.001) ? myFade : 1.0;',
    '',
    '    float alpha;',
    '    if (inTentacle > 0.0) {',
