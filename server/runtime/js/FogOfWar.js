...
var EDGE_DISSOLVE_FRAG = [
    ...
    '',
    '    // 현재 타일의 NEAREST vis (보간 없이 정확한 타일 값)',
    '    float myVis = sampleVisNearest(uv, texel);',
    '    float explored = sampleExplNearest(uv, texel);',
    '',
    '    // 인접 8방향의 vis 샘플링 (NEAREST)',
    '    float vL  = sampleVisNearest(uv + vec2(-texel.x, 0.0), texel);',
    '    float vR  = sampleVisNearest(uv + vec2( texel.x, 0.0), texel);',
    '    float vU  = sampleVisNearest(uv + vec2(0.0, -texel.y), texel);',
    '    float vD  = sampleVisNearest(uv + vec2(0.0,  texel.y), texel);',
    '    float vLU = sampleVisNearest(uv + vec2(-texel.x, -texel.y), texel);',
    '    float vRU = sampleVisNearest(uv + vec2( texel.x, -texel.y), texel);',
    '    float vLD = sampleVisNearest(uv + vec2(-texel.x,  texel.y), texel);',
    '    float vRD = sampleVisNearest(uv + vec2( texel.x,  texel.y), texel);',
    '',
    '    // 타일 내부 픽셀 위치 (0~1)',
    '    vec2 tp = fract(uv * mapSize);',
    '',
    '    // 시야↔미탐험 경계까지의 부호 있는 거리 계산',
    '    // 양수: 시야쪽(안쪽), 음수: 미탐험쪽(바깥쪽)',
    '    float signedDist;',
    '',
    '    if (myVis > 0.01) {',
    '        // 시야 타일: vis=0 인접까지의 거리 (양수)',
    '        bool hasUnexplored = (vL < 0.01 || vR < 0.01 || vU < 0.01 || vD < 0.01',
    '                           || vLU < 0.01 || vRU < 0.01 || vLD < 0.01 || vRD < 0.01);',
    '        if (!hasUnexplored) discard;',
    '',
    '        float minD = 1.0;',
    '        if (vL  < 0.01) minD = min(minD, tp.x);',
    '        if (vR  < 0.01) minD = min(minD, 1.0 - tp.x);',
    '        if (vU  < 0.01) minD = min(minD, tp.y);',
    '        if (vD  < 0.01) minD = min(minD, 1.0 - tp.y);',
    '        if (vLU < 0.01) minD = min(minD, length(tp));',
    '        if (vRU < 0.01) minD = min(minD, length(vec2(1.0 - tp.x, tp.y)));',
    '        if (vLD < 0.01) minD = min(minD, length(vec2(tp.x, 1.0 - tp.y)));',
    '        if (vRD < 0.01) minD = min(minD, length(1.0 - tp));',
    '        signedDist = minD;',
    '    } else {',
    '        // 미탐험 타일: vis>0 인접까지의 거리 (음수)',
    '        bool hasVisible = (vL > 0.01 || vR > 0.01 || vU > 0.01 || vD > 0.01',
    '                        || vLU > 0.01 || vRU > 0.01 || vLD > 0.01 || vRD > 0.01);',
    '        if (!hasVisible) discard;',
    '',
    '        float minD = 1.0;',
    '        if (vL  > 0.01) minD = min(minD, tp.x);',
    '        if (vR  > 0.01) minD = min(minD, 1.0 - tp.x);',
    ...
    '',
    '    // adjustedDist < 0이면 미탐험쪽 → fog가 이미 처리 → discard',
    '    // (미탐험 타일에서 edgeOffset이 충분히 크면 경계로 포함됨)',
    '    if (adjustedDist < 0.0) discard;',
    '',
    '    // distNorm: 0(경계 기준점) ~ 1(시야 안쪽)',
    ...
    '',
    '    // 방향성 디졸브: distNorm에 비례하여 노이즈 적용',
    '    // 시야쪽(distNorm≈1): 노이즈 100% → 불규칙한 알파',
    '    // 미탐험쪽(distNorm≈0): 노이즈 0% → 항상 불투명',
    '    float directedDn = dn * distNorm;',
    '',
    '    // 기본 알파: 미탐험 가장자리(distNorm=0)에서 1, 시야 안쪽(distNorm=1)에서 0',
    '    float baseT = 1.0 - distNorm;',
    '    // 디졸브로 불투명 영역 확장 (시야쪽에서만 불규칙하게)',
