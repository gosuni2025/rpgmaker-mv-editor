...
var VOL_FOG_FRAG = [
    ...
    '',
    '    // 디졸브 경계: visibility를 노이즈로 변조 → 경계가 울퉁불퉁해짐',
    '    // visibility > 0인 영역에서만 적용 (미탐험 침투 방지)',
    '    float dissolvedVis = visibility;',
    '    if (visibility > 0.001) {',
    '        dissolvedVis = visibility + dn * 0.5;',
    '        dissolvedVis = max(dissolvedVis, 0.0);', // 음수 방지
    '    }',
    '',
    '    // 최종 fog alpha 계산',
    '    float fogAlpha;',
    '    if (dissolvedVis > 0.01) {',
    '        // 시야 내: 디졸브된 가시성으로 alpha 계산',
    '        float visFactor = clamp(dissolvedVis + visibilityBrightness * dissolvedVis, 0.0, 1.0);',
    '        fogAlpha = exploredAlpha * (1.0 - visFactor);',
    '    } else if (explored > 0.01) {',
    '        // 탐험완료: explored 보간값으로 미탐험↔탐험 경계 자연스럽게',
    '        float baseAlpha = mix(unexploredAlpha, exploredAlpha, explored);',
    '        float explEdge = explored * (1.0 - explored) * 4.0;',
    '        fogAlpha = baseAlpha + dn * 0.12 * explEdge;',
    '    } else {',
    '        // 미탐험: 완전 불투명 (디졸브 없음)',
