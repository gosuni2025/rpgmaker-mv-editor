...
var EDGE_DISSOLVE_FRAG = [
    ...
    '        if (isBorder(uvL, texel)) { distToBorder = min(distToBorder, (r - 1.0) + tp.x); foundBorder = true; }',
    '        if (isBorder(uvR, texel)) { distToBorder = min(distToBorder, (r - 1.0) + (1.0 - tp.x)); foundBorder = true; }',
    '        if (isBorder(uvU, texel)) { distToBorder = min(distToBorder, (r - 1.0) + tp.y); foundBorder = true; }',
    '        if (isBorder(uvD, texel)) { distToBorder = min(distToBorder, (r - 1.0) + (1.0 - tp.y)); foundBorder = true; }',
    '        // 대각선',
    ...
    '        if (isBorder(uvLU, texel)) { distToBorder = min(distToBorder, (r - 1.0) * diag + length(tp)); foundBorder = true; }',
    '        if (isBorder(uvRU, texel)) { distToBorder = min(distToBorder, (r - 1.0) * diag + length(vec2(1.0 - tp.x, tp.y))); foundBorder = true; }',
    '        if (isBorder(uvLD, texel)) { distToBorder = min(distToBorder, (r - 1.0) * diag + length(vec2(tp.x, 1.0 - tp.y))); foundBorder = true; }',
    '        if (isBorder(uvRD, texel)) { distToBorder = min(distToBorder, (r - 1.0) * diag + length(vec2(1.0 - tp.x, 1.0 - tp.y))); foundBorder = true; }',
    '    }',
    '',
    '    // fadeFactor: bilinear 보간된 fade값 (격자 방지)',
    '    // 미탐험 경계에서는 1.0, fade 중인 영역에서는 0~1 부드러운 그라데이션',
    '    float fadeFactor = myExpl > 0.5 ? myFade : 1.0;',
    '    // 인접에서 가장 높은 fade도 반영 (경계 근처에서 부드럽게)',
    '    if (foundBorder && fadeFactor < 0.01) {',
    '        // 경계를 찾았지만 현재 위치의 fade가 0: 인접 경계의 fade를 bilinear로 추정',
    '        fadeFactor = sampleFadeBilinear(uv);',
    '    }',
    '',
    '    if (!foundBorder && fadeFactor < 0.01) discard;',
    '',
    '    float timeS = uTime * edgeAnimSpeed;',
    '',
