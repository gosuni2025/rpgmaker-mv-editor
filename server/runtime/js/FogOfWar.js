...
var EDGE_DISSOLVE_FRAG = [
    ...
    '',
    '    // 미탐험 방향 벡터: 인접 8방향에서 미탐험인 쪽으로 향하는 벡터 합산',
    '    vec2 unexplDir = vec2(0.0);',
    '    float inv = 0.7071;',  // 1/sqrt(2) 대각선 정규화
    '    if (eL  < 0.5) unexplDir += vec2(-1.0, 0.0);',
    '    if (eR  < 0.5) unexplDir += vec2( 1.0, 0.0);',
    '    if (eU  < 0.5) unexplDir += vec2(0.0, -1.0);',
    '    if (eD  < 0.5) unexplDir += vec2(0.0,  1.0);',
    '    if (eLU < 0.5) unexplDir += vec2(-inv, -inv);',
    '    if (eRU < 0.5) unexplDir += vec2( inv, -inv);',
    '    if (eLD < 0.5) unexplDir += vec2(-inv,  inv);',
    '    if (eRD < 0.5) unexplDir += vec2( inv,  inv);',
    '    float unexplLen = length(unexplDir);',
    '    if (unexplLen > 0.001) unexplDir /= unexplLen;',
    '',
    '    // 픽셀의 타일 중심 기준 위치 (-0.5 ~ 0.5)',
    '    vec2 tpCentered = tp - 0.5;',
    '    // 미탐험 방향과의 정렬도: -1(탐험쪽) ~ +1(미탐험쪽)',
    '    float dirAlign = dot(normalize(tpCentered + 0.001), unexplDir);',
    '    // 방향성 계수: 미탐험쪽(dirAlign>0)이면 0에 가깝게, 탐험쪽이면 1에 가깝게',
    ...
    '',
    '    // 방향성 디졸브: dirFactor로 방향별 노이즈 강도 조절',
    '    // 탐험쪽(dirFactor≈1): 노이즈 강하게 → 불규칙 투명',
    '    // 미탐험쪽(dirFactor≈0): 노이즈 없음 → 항상 불투명',
    '    float directedDn = dn * dirFactor;',
    '',
    '    // 기본 알파: 경계(distNorm=0)에서 1, 탐험 안쪽(distNorm=1)에서 0',
    '    float baseT = 1.0 - distNorm;',
    '    // 디졸브로 불투명 영역 조절 (탐험쪽에서만 불규칙하게)',
