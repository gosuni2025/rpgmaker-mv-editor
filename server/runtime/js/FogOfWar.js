...
var EDGE_DISSOLVE_FRAG = [
    ...
    '',
    '    // 가장 가까운 경계(미탐험 or fade중) 타일까지의 거리',
    '    float distToBorder = 99.0;',
    '    bool foundBorder = false;',
    '    bool foundRealBorder = false;', // 실제 미탐험 경계 유무
    '    float closestFade = 0.0;',     // 가장 가까운 경계의 fade값
    '    float diag = 1.414;',
    '    for (float r = 1.0; r <= 4.0; r += 1.0) {',
    ...
    '        // 4방향',
    '        if (isBorder(uvL, texel)) { float d = (r - 1.0) + tp.x; if (d < distToBorder) { distToBorder = d; closestFade = borderFade(uvL, texel); } foundBorder = true; if (sampleExplNearest(uvL, texel) < 0.5) foundRealBorder = true; }',
    '        if (isBorder(uvR, texel)) { float d = (r - 1.0) + (1.0 - tp.x); if (d < distToBorder) { distToBorder = d; closestFade = borderFade(uvR, texel); } foundBorder = true; if (sampleExplNearest(uvR, texel) < 0.5) foundRealBorder = true; }',
    '        if (isBorder(uvU, texel)) { float d = (r - 1.0) + tp.y; if (d < distToBorder) { distToBorder = d; closestFade = borderFade(uvU, texel); } foundBorder = true; if (sampleExplNearest(uvU, texel) < 0.5) foundRealBorder = true; }',
    '        if (isBorder(uvD, texel)) { float d = (r - 1.0) + (1.0 - tp.y); if (d < distToBorder) { distToBorder = d; closestFade = borderFade(uvD, texel); } foundBorder = true; if (sampleExplNearest(uvD, texel) < 0.5) foundRealBorder = true; }',
    '        // 대각선',
    ...
    '        if (isBorder(uvLU, texel)) { float d = (r - 1.0) * diag + length(tp); if (d < distToBorder) { distToBorder = d; closestFade = borderFade(uvLU, texel); } foundBorder = true; if (sampleExplNearest(uvLU, texel) < 0.5) foundRealBorder = true; }',
    '        if (isBorder(uvRU, texel)) { float d = (r - 1.0) * diag + length(vec2(1.0 - tp.x, tp.y)); if (d < distToBorder) { distToBorder = d; closestFade = borderFade(uvRU, texel); } foundBorder = true; if (sampleExplNearest(uvRU, texel) < 0.5) foundRealBorder = true; }',
    '        if (isBorder(uvLD, texel)) { float d = (r - 1.0) * diag + length(vec2(tp.x, 1.0 - tp.y)); if (d < distToBorder) { distToBorder = d; closestFade = borderFade(uvLD, texel); } foundBorder = true; if (sampleExplNearest(uvLD, texel) < 0.5) foundRealBorder = true; }',
    '        if (isBorder(uvRD, texel)) { float d = (r - 1.0) * diag + length(vec2(1.0 - tp.x, 1.0 - tp.y)); if (d < distToBorder) { distToBorder = d; closestFade = borderFade(uvRD, texel); } foundBorder = true; if (sampleExplNearest(uvRD, texel) < 0.5) foundRealBorder = true; }',
    '    }',
    '',
    '    if (!foundBorder) discard;',
    '',
    '    // fadeFactor: 실제 미탐험 경계면 1.0, fade중이면 bilinear로 부드럽게',
    ...
    '    } else if (foundRealBorder) {',
    '        // 실제 미탐험 경계 근처: exploredAlpha로 채움',
    '        alpha = exploredAlpha;',
    '    } else {',
    '        // fade중인 경계 근처: fadeFactor로 감쇄',
