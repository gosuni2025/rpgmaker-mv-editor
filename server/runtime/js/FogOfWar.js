...
var EDGE_DISSOLVE_FRAG = [
    ...
    '',
    '    // 시야 타일 위에서만 동작 (시야 안쪽 가장자리를 fog로 덮음)',
    '    if (myVis < 0.01) discard;',
    '',
    ...
    '',
    '    // 인접에 시야 밖(vis=0) 타일이 없으면 경계가 아님 → discard',
    ...
    '',
    '    // vis=0인 인접 타일 방향의 가장자리/모서리까지 최소 거리',
    '    // 이 거리가 작을수록 미탐험에 가까움 → 불투명',
    '    float minDist = 1.0;',
    ...
    '',
    '    // minDist 0(미탐험 가장자리)~nearVisWeight(시야 안쪽)',
    '    // distNorm: 0(미탐험 가장자리) ~ 1(시야 안쪽)',
    '    float distNorm = clamp(minDist / nearVisWeight, 0.0, 1.0);',
    '',
    '    // 디졸브 노이즈',
    '    float timeS = uTime * edgeAnimSpeed;',
