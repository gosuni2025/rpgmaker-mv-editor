...
var EDGE_DISSOLVE_FRAG = [
    ...
    '        // 4방향',
    '        if (isBorder(uvL, texel)) { float d = (r - 1.0) + tp.x; if (d < distToBorder) { distToBorder = d; closestFade = borderFade(uvL, texel); } foundBorder = true; }',
    '        if (isBorder(uvR, texel)) { float d = (r - 1.0) + (1.0 - tp.x); if (d < distToBorder) { distToBorder = d; closestFade = borderFade(uvR, texel); } foundBorder = true; }',
    '        if (isBorder(uvU, texel)) { float d = (r - 1.0) + tp.y; if (d < distToBorder) { distToBorder = d; closestFade = borderFade(uvU, texel); } foundBorder = true; }',
    '        if (isBorder(uvD, texel)) { float d = (r - 1.0) + (1.0 - tp.y); if (d < distToBorder) { distToBorder = d; closestFade = borderFade(uvD, texel); } foundBorder = true; }',
    '        // 대각선',
    ...
    '        if (isBorder(uvLU, texel)) { float d = (r - 1.0) * diag + length(tp); if (d < distToBorder) { distToBorder = d; closestFade = borderFade(uvLU, texel); } foundBorder = true; }',
    '        if (isBorder(uvRU, texel)) { float d = (r - 1.0) * diag + length(vec2(1.0 - tp.x, tp.y)); if (d < distToBorder) { distToBorder = d; closestFade = borderFade(uvRU, texel); } foundBorder = true; }',
    '        if (isBorder(uvLD, texel)) { float d = (r - 1.0) * diag + length(vec2(tp.x, 1.0 - tp.y)); if (d < distToBorder) { distToBorder = d; closestFade = borderFade(uvLD, texel); } foundBorder = true; }',
    '        if (isBorder(uvRD, texel)) { float d = (r - 1.0) * diag + length(vec2(1.0 - tp.x, 1.0 - tp.y)); if (d < distToBorder) { distToBorder = d; closestFade = borderFade(uvRD, texel); } foundBorder = true; }',
    '    }',
    '',
    '    if (!foundBorder) discard;',
    '',
    '    // fadeFactor: 가장 가까운 경계의 fade값 사용',
    '    // closestFade = 1.0 (실제 미탐험), 0~1 (fade 중인 탐험 타일)',
    '    float fadeFactor = closestFade;',
    '',
    '    float timeS = uTime * edgeAnimSpeed;',
    '',
    '    // 촉수 노이즈',
    ...
    '        // 실제 미탐험 경계 근처 (촉수 바깥): exploredAlpha로 부드럽게 전환',
    '        alpha = exploredAlpha;',
    '    } else {',
    '        discard;',
    '    }',
