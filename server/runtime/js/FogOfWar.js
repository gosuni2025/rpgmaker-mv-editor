...
var EDGE_DISSOLVE_FRAG = [
    ...
    '',
    '    // 실제 미탐험 경계와 fade 경계를 분리하여 추적',
    '    float distToReal = 99.0;',   // 실제 미탐험 타일까지의 거리
    '    float distToFade = 99.0;',   // fade 중인 타일까지의 거리
    '    float closestFadeVal = 0.0;', // fade 중인 타일의 fade 값
    ...
    '        // 4방향 — 실제 미탐험 vs fade 분리',
    '        if (isBorder(uvL, texel)) { float d = (r - 1.0) + tp.x; float bf = borderFade(uvL, texel); if (bf >= 1.0) { if (d < distToReal) distToReal = d; } else { if (d < distToFade) { distToFade = d; closestFadeVal = bf; } } foundBorder = true; }',
    '        if (isBorder(uvR, texel)) { float d = (r - 1.0) + (1.0 - tp.x); float bf = borderFade(uvR, texel); if (bf >= 1.0) { if (d < distToReal) distToReal = d; } else { if (d < distToFade) { distToFade = d; closestFadeVal = bf; } } foundBorder = true; }',
    '        if (isBorder(uvU, texel)) { float d = (r - 1.0) + tp.y; float bf = borderFade(uvU, texel); if (bf >= 1.0) { if (d < distToReal) distToReal = d; } else { if (d < distToFade) { distToFade = d; closestFadeVal = bf; } } foundBorder = true; }',
    '        if (isBorder(uvD, texel)) { float d = (r - 1.0) + (1.0 - tp.y); float bf = borderFade(uvD, texel); if (bf >= 1.0) { if (d < distToReal) distToReal = d; } else { if (d < distToFade) { distToFade = d; closestFadeVal = bf; } } foundBorder = true; }',
    '        // 대각선',
    ...
    '        if (isBorder(uvLU, texel)) { float d = (r - 1.0) * diag + length(tp); float bf = borderFade(uvLU, texel); if (bf >= 1.0) { if (d < distToReal) distToReal = d; } else { if (d < distToFade) { distToFade = d; closestFadeVal = bf; } } foundBorder = true; }',
    '        if (isBorder(uvRU, texel)) { float d = (r - 1.0) * diag + length(vec2(1.0 - tp.x, tp.y)); float bf = borderFade(uvRU, texel); if (bf >= 1.0) { if (d < distToReal) distToReal = d; } else { if (d < distToFade) { distToFade = d; closestFadeVal = bf; } } foundBorder = true; }',
    '        if (isBorder(uvLD, texel)) { float d = (r - 1.0) * diag + length(vec2(tp.x, 1.0 - tp.y)); float bf = borderFade(uvLD, texel); if (bf >= 1.0) { if (d < distToReal) distToReal = d; } else { if (d < distToFade) { distToFade = d; closestFadeVal = bf; } } foundBorder = true; }',
    '        if (isBorder(uvRD, texel)) { float d = (r - 1.0) * diag + length(vec2(1.0 - tp.x, 1.0 - tp.y)); float bf = borderFade(uvRD, texel); if (bf >= 1.0) { if (d < distToReal) distToReal = d; } else { if (d < distToFade) { distToFade = d; closestFadeVal = bf; } } foundBorder = true; }',
    '    }',
    '',
    '    if (!foundBorder) discard;',
    '',
    '    // distToBorder & fadeFactor 결정',
    '    // 실제 미탐험: 촉수 모양/길이 기준선',
    '    // fade 타일: 촉수 페이드 아웃 영향도',
    '    float distToBorder;',
    '    float fadeFactor;',
    '    bool onlyFade = false;',
    '    if (distToReal < 99.0 && distToFade < 99.0) {',
    '        // 실제 미탐험 + fade 모두 존재',
    '        // distToBorder는 실제 미탐험 기준 (안정적 촉수 모양)',
    '        distToBorder = distToReal;',
    '        // fade 타일이 가까울수록 fadeFactor에 영향 (방향성)',
    '        // fade가 멀수록 (fadeInfluence→1.0) fadeFactor → 1.0 (촉수 유지)',
    '        // fade가 가까울수록 (fadeInfluence→0.0) fadeFactor → closestFadeVal (촉수 줄어듦)',
    '        float fadeInfluence = smoothstep(0.0, 3.0, distToFade);',
    '        fadeFactor = mix(closestFadeVal, 1.0, fadeInfluence);',
    '    } else if (distToReal < 99.0) {',
    '        // 실제 미탐험만 존재 → 촉수 완전 유지',
    '        distToBorder = distToReal;',
    '        fadeFactor = 1.0;',
    '    } else {',
    '        // fade 경계만 존재 → 촉수를 fade에 따라 줄임',
    '        distToBorder = distToFade;',
    '        fadeFactor = closestFadeVal;',
    '        onlyFade = true;',
    '    }',
    '',
    '    float timeS = uTime * edgeAnimSpeed;',
    ...
    '    // 촉수 전체 길이 (fade와 무관하게 일단 계산)',
    '    float fullLength = pow(nNorm, tentacleSharpness) * dissolveStrength;',
    '    // fadeFactor로 촉수의 최대 도달 길이를 줄임 → 끝에서부터 줄어듦',
    ...
    '        // 촉수 끝부분(경계에서 먼 곳)일수록 투명하게: 자연스러운 소멸',
    '        if (fadeFactor < 1.0) {',
    '            // 촉수 끝의 비율: 0(뿌리)~1(끝) — 끝에 가까울수록 투명',
    '            float tipRatio = distToBorder / max(tentacleLength, 0.01);',
    '            alpha *= mix(fadeFactor, 1.0, 1.0 - tipRatio);',
    '        }',
    '        alpha *= unexploredAlpha;',
    '    } else {',
    '        // 촉수 바깥 영역',
    '        if (onlyFade) {',
    '            // fade만 존재: fade값에 따라 exploredAlpha 감쇄',
    '            alpha = exploredAlpha * fadeFactor;',
    '        } else {',
    '            // 실제 미탐험 존재: 정상 fog (exploredAlpha)',
    '            alpha = exploredAlpha;',
    '        }',
    '    }',
    '',
