...
var VOL_FOG_FRAG = [
    ...
    '',
    '    // 탐험↔미탐험 경계 타일은 edge dissolve 메시가 담당 → discard',
    '    if (outsideDist < 0.001) {',
...
var EDGE_DISSOLVE_FRAG = [
    ...
    '    vec2 texel = 1.0 / mapSize;',
    '',
    '    // 현재 타일의 explored 상태 (G채널) — 경계 판단 기준',
    '    float myExpl = sampleExplNearest(uv, texel);',
    '',
    '    // 인접 8방향의 explored 샘플링 (NEAREST)',
    ...
    '',
    '    // 탐험↔미탐험 경계까지의 부호 있는 거리 계산',
    '    // 양수: 탐험쪽(안쪽), 음수: 미탐험쪽(바깥쪽)',
    '    float signedDist;',
    '',
    '    if (myExpl > 0.5) {',
    '        // 탐험 타일: 미탐험(explored=0) 인접까지의 거리 (양수)',
    '        bool hasUnexplored = (eL < 0.5 || eR < 0.5 || eU < 0.5 || eD < 0.5',
    '                           || eLU < 0.5 || eRU < 0.5 || eLD < 0.5 || eRD < 0.5);',
    '        if (!hasUnexplored) discard;',
    '',
    '        float minD = 1.0;',
    '        if (eL  < 0.5) minD = min(minD, tp.x);',
    '        if (eR  < 0.5) minD = min(minD, 1.0 - tp.x);',
    '        if (eU  < 0.5) minD = min(minD, tp.y);',
    '        if (eD  < 0.5) minD = min(minD, 1.0 - tp.y);',
    '        // 대각선: 코너를 사각형화 — 두 cardinal 방향 각각 경계 추가',
    '        if (eLU < 0.5) { minD = min(minD, tp.x); minD = min(minD, tp.y); }',
    '        if (eRU < 0.5) { minD = min(minD, 1.0 - tp.x); minD = min(minD, tp.y); }',
    '        if (eLD < 0.5) { minD = min(minD, tp.x); minD = min(minD, 1.0 - tp.y); }',
    '        if (eRD < 0.5) { minD = min(minD, 1.0 - tp.x); minD = min(minD, 1.0 - tp.y); }',
    '        signedDist = minD;',
    '    } else {',
    '        // 미탐험 타일: 탐험(explored>0) 인접까지의 거리 (음수)',
    '        bool hasExplored = (eL > 0.5 || eR > 0.5 || eU > 0.5 || eD > 0.5',
    '                         || eLU > 0.5 || eRU > 0.5 || eLD > 0.5 || eRD > 0.5);',
    '        if (!hasExplored) discard;',
    '',
    '        float minD = 1.0;',
    '        if (eL  > 0.5) minD = min(minD, tp.x);',
    '        if (eR  > 0.5) minD = min(minD, 1.0 - tp.x);',
    ...
    '    }',
    '',
    '    // edgeOffset로 경계 위치 이동: 양수=바깥으로, 음수=안쪽으로',
    '    float adjustedDist = signedDist + edgeOffset;',
    '',
    '    // adjustedDist < 0이면 미탐험쪽 깊숙한 곳 → fog 메시가 처리 → discard',
    '    // (미탐험 타일에서도 edgeOffset이 충분히 크면 디졸브 경계에 포함됨)',
    '    if (adjustedDist < 0.0) discard;',
    '',
    '    // distNorm: 0(경계 기준점) ~ 1(탐험 안쪽)',
    '    float distNorm = clamp(adjustedDist / nearVisWeight, 0.0, 1.0);',
    '',
    '    // 디졸브 노이즈',
    '    float timeS = uTime * edgeAnimSpeed;',
    ...
    '',
    '    // 방향성: 픽셀이 경계(미탐험 인접) 방향으로 치우쳤으면 불투명 유지',
    '    // 반대(탐험 안쪽) 방향이면 디졸브 적용하여 불규칙 투명',
    '    vec2 tpCentered = tp - 0.5;',  // -0.5 ~ 0.5
    '    float wL = max(-tpCentered.x, 0.0) * 2.0;',
    '    float wR = max( tpCentered.x, 0.0) * 2.0;',
    '    float wU = max(-tpCentered.y, 0.0) * 2.0;',
    '    float wD = max( tpCentered.y, 0.0) * 2.0;',
    '    // 경계 반대편 판정 (탐험 타일→미탐험 인접, 미탐험 타일→탐험 인접)',
    '    float bL  = (myExpl > 0.5) ? ((eL  < 0.5) ? 1.0 : 0.0) : ((eL  > 0.5) ? 1.0 : 0.0);',
    '    float bR  = (myExpl > 0.5) ? ((eR  < 0.5) ? 1.0 : 0.0) : ((eR  > 0.5) ? 1.0 : 0.0);',
    '    float bU  = (myExpl > 0.5) ? ((eU  < 0.5) ? 1.0 : 0.0) : ((eU  > 0.5) ? 1.0 : 0.0);',
    '    float bD  = (myExpl > 0.5) ? ((eD  < 0.5) ? 1.0 : 0.0) : ((eD  > 0.5) ? 1.0 : 0.0);',
    '    float bLU = (myExpl > 0.5) ? ((eLU < 0.5) ? 1.0 : 0.0) : ((eLU > 0.5) ? 1.0 : 0.0);',
    '    float bRU = (myExpl > 0.5) ? ((eRU < 0.5) ? 1.0 : 0.0) : ((eRU > 0.5) ? 1.0 : 0.0);',
    '    float bLD = (myExpl > 0.5) ? ((eLD < 0.5) ? 1.0 : 0.0) : ((eLD > 0.5) ? 1.0 : 0.0);',
    '    float bRD = (myExpl > 0.5) ? ((eRD < 0.5) ? 1.0 : 0.0) : ((eRD > 0.5) ? 1.0 : 0.0);',
    '    // 대각선 가중치: 두 cardinal 방향의 min (코너 픽셀만 영향)',
    '    float wLU = min(wL, wU); float wRU = min(wR, wU);',
    '    float wLD = min(wL, wD); float wRD = min(wR, wD);',
    '    float blockW = bL*wL + bR*wR + bU*wU + bD*wD + bLU*wLU + bRU*wRU + bLD*wLD + bRD*wRD;',
    '    float totalW = wL + wR + wU + wD + wLU + wRU + wLD + wRD;',
    '    // dirFactor: 0(경계 방향=불투명) ~ 1(안쪽 방향=디졸브)',
    '    float dirFactor = (totalW > 0.01) ? 1.0 - blockW / totalW : 0.5;',
    '',
    '    // 기본 알파: 경계(distNorm=0)에서 1, 탐험 안쪽(distNorm=1)에서 0',
    '    float baseT = 1.0 - distNorm;',
    '    // 경계 방향(dirFactor≈0): baseT 유지 → 불투명',
    '    // 안쪽 방향(dirFactor≈1): 디졸브 노이즈가 baseT를 줄여서 투명하게',
    '    float edgeVal = clamp(baseT - dn * dirFactor, 0.0, 1.0);',
    '',
    '    // edgeVal 클수록 불투명',
    '    float baseAlpha = (myExpl > 0.5) ? exploredAlpha : unexploredAlpha;',
