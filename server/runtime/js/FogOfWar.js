...
var VOL_FOG_FRAG = [
    ...
    '',
    '    float dissolveAmt = edgeAnimOn * dissolveNoise;',
    '',
    '    // 경계 감지: LinearFilter 보간으로 텍셀 경계에서 중간값 발생',
    '    // visibility 경계: 0 < vis < 1 (시야 가장자리)',
    '    // explored 경계: 0 < expl < 1 (탐험 가장자리, LinearFilter 보간)',
    '    float visEdge = smoothstep(0.0, 0.15, visibility) * (1.0 - smoothstep(0.6, 1.0, visibility));',
    '    float explEdge = explored * (1.0 - explored) * 4.0;', // 0.5일 때 최대 1.0
    '',
    '    // 최종 fog alpha 계산',
    '    float fogAlpha;',
    '    if (visibility > 0.01) {',
    '        // 시야 내: 경계에서만 디졸브 적용',
    '        float visFactor = clamp(visibility + dissolveAmt * 0.6 * visEdge + visibilityBrightness * visibility, 0.0, 1.0);',
    '        fogAlpha = exploredAlpha * (1.0 - visFactor);',
    '    } else if (explored > 0.01) {',
    '        // 탐험완료 (보간 포함): 미탐험 경계에서 디졸브',
    '        float baseAlpha = mix(unexploredAlpha, exploredAlpha, explored);',
    '        fogAlpha = baseAlpha + dissolveAmt * 0.15 * explEdge;',
    '    } else {',
    '        // 미탐험: 완전 불투명 (디졸브 없음)',
    ...
    '        float glowFactor = visibility * visibilityBrightness;',
    '        color += vec3(0.15, 0.12, 0.08) * glowFactor;',
    '    }',
    '',
