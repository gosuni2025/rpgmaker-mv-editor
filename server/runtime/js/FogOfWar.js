...
var VOL_FOG_FRAG = [
    ...
    '        if (myE > 0.5) {',
    '            // 탐험 타일: 인접에 unexplored가 있으면 경계 → discard',
    ...
    '            bool borderTile = (eL < 0.5 || eR < 0.5 || eU < 0.5 || eD < 0.5',
    '                            || eLU < 0.5 || eRU < 0.5 || eLD < 0.5 || eRD < 0.5);',
    '            if (borderTile) discard;',
    '        }',
    '    }',
    '',
    '    // 시야 밖: 불투명',
...
var EDGE_DISSOLVE_FRAG = [
    ...
    '    // 탐험쪽(distNorm≈1): 노이즈 100% → 불규칙한 알파',
    '    // 미탐험쪽(distNorm≈0): 노이즈 0% → 항상 불투명',
    '    float directedDn = dn * distNorm;',
    '',
    '    // 기본 알파: 미탐험 가장자리(distNorm=0)에서 1, 탐험 안쪽(distNorm=1)에서 0',
    '    float baseT = 1.0 - distNorm;',
    '    // 디졸브로 불투명 영역 확장 (탐험쪽에서만 불규칙하게)',
