...
var VOL_FOG_FRAG = [
    ...
    '    }',
    '',
    '    // --- 디졸브 경계: 다중 옥타브 노이즈로 시야 경계를 불규칙하게 ---',
    '    float timeS = uTime * edgeAnimSpeed;',
    '',
    '    // 큰 스케일 노이즈 (느리게 움직임) + 작은 스케일 노이즈 (빠르게 디테일)',
    ...
    '',
    '    // 시야 경계 디졸브: visibility에 노이즈를 더해서 경계를 울퉁불퉁하게',
    '    float dissolveStrength = edgeAnimOn * 0.6;', // 디졸브 강도 (더 불규칙하게)
    '    float adjustedVis = visibility + dissolveNoise * dissolveStrength;',
    '    float adjustedExpl = explored + dissolveNoise * dissolveStrength * 0.8;', // 탐험 경계도 디졸브
    '',
    '    // 최종 fog alpha 계산',
    '    float fogAlpha;',
    '    if (adjustedVis > 0.01) {',
    '        // 시야 내: 디졸브 적용된 가시성',
    '        float visFactor = clamp(adjustedVis + visibilityBrightness * visibility, 0.0, 1.0);',
    '        fogAlpha = exploredAlpha * (1.0 - visFactor);',
    '    } else if (adjustedExpl > 0.5) {',
    '        // 탐험완료: exploredAlpha + 경계 디졸브',
    '        fogAlpha = exploredAlpha + dissolveNoise * 0.08;',
    '    } else {',
    '        // 미탐험: 완전 불투명 (unexploredAlpha)',
