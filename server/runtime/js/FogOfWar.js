...
var EDGE_DISSOLVE_FRAG = [
    ...
    '',
    '    // 방향성 디졸브: 픽셀이 타일 중심 기준으로 어느 방향에 있는지 판단',
    '    // 그 방향의 인접 타일이 미탐험이면 디졸브 억제 (벽 쪽은 항상 불투명)',
    '    vec2 tpCentered = tp - 0.5;',  // -0.5 ~ 0.5
    '    // 4방향 가중치: 픽셀이 각 방향으로 얼마나 치우쳤는지 (0~1)',
    '    float wL = max(-tpCentered.x, 0.0) * 2.0;',  // 왼쪽
    '    float wR = max( tpCentered.x, 0.0) * 2.0;',  // 오른쪽
    '    float wU = max(-tpCentered.y, 0.0) * 2.0;',  // 위
    '    float wD = max( tpCentered.y, 0.0) * 2.0;',  // 아래
    '    // 각 방향에 미탐험 인접이 있으면 그 방향의 가중치만큼 억제',
    '    float blockSum = 0.0;',
    '    float weightSum = 0.0;',
    '    if (eL < 0.5) { blockSum += wL; } weightSum += wL;',
    '    if (eR < 0.5) { blockSum += wR; } weightSum += wR;',
    '    if (eU < 0.5) { blockSum += wU; } weightSum += wU;',
    '    if (eD < 0.5) { blockSum += wD; } weightSum += wD;',
    '    // dirFactor: 0(미탐험 방향=억제) ~ 1(탐험 방향=디졸브 강하게)',
    '    float dirFactor = (weightSum > 0.001) ? 1.0 - blockSum / weightSum : 1.0;',
    '',
    '    // 디졸브 노이즈',
    '    float timeS = uTime * edgeAnimSpeed;',
