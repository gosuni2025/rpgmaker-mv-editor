...
var EDGE_DISSOLVE_FRAG = [
    ...
    '',
    '    // 경계 방향 벡터: 인접 8방향에서 경계 반대편(억제할 쪽) 방향 합산',
    '    // 탐험 타일: 미탐험 인접 방향, 미탐험 타일: 탐험 인접 방향',
    '    vec2 boundaryDir = vec2(0.0);',
    '    float inv = 0.7071;',  // 1/sqrt(2)
    '    if (myExpl > 0.5) {',
    '        if (eL  < 0.5) boundaryDir += vec2(-1.0, 0.0);',
    '        if (eR  < 0.5) boundaryDir += vec2( 1.0, 0.0);',
    '        if (eU  < 0.5) boundaryDir += vec2(0.0, -1.0);',
    '        if (eD  < 0.5) boundaryDir += vec2(0.0,  1.0);',
    '        if (eLU < 0.5) boundaryDir += vec2(-inv, -inv);',
    '        if (eRU < 0.5) boundaryDir += vec2( inv, -inv);',
    '        if (eLD < 0.5) boundaryDir += vec2(-inv,  inv);',
    '        if (eRD < 0.5) boundaryDir += vec2( inv,  inv);',
    '    } else {',
    '        if (eL  > 0.5) boundaryDir += vec2(-1.0, 0.0);',
    '        if (eR  > 0.5) boundaryDir += vec2( 1.0, 0.0);',
    '        if (eU  > 0.5) boundaryDir += vec2(0.0, -1.0);',
    ...
    '        if (eRD > 0.5) boundaryDir += vec2( inv,  inv);',
    '    }',
    '    float bLen = length(boundaryDir);',
    '    if (bLen > 0.001) boundaryDir /= bLen;',
    '',
    '    // 픽셀의 타일 중심 기준 위치 (-0.5 ~ 0.5)',
    '    vec2 tpCentered = tp - 0.5;',
    '    // 경계 방향과의 정렬도: +1(경계쪽) ~ -1(안쪽)',
    '    float dirAlign = dot(normalize(tpCentered + 0.001), boundaryDir);',
    '    // dirFactor: 경계쪽(dirAlign>0)이면 0, 안쪽이면 1',
    '    float dirFactor = clamp(0.5 - dirAlign * 0.5, 0.0, 1.0);',
    '',
    '    // 디졸브 노이즈',
    '    float timeS = uTime * edgeAnimSpeed;',
    ...
    '',
    '    // 방향성 디졸브: dirFactor로 노이즈 강도 조절',
    '    // 안쪽(dirFactor≈1): 노이즈 강하게 → 불규칙 불투명',
    '    // 경계쪽(dirFactor≈0): 노이즈 없음 → 항상 불투명',
    '    float directedDn = dn * dirFactor;',
    '',
    '    // 기본 알파: 경계(distNorm=0)에서 1, 탐험 안쪽(distNorm=1)에서 0',
    '    float baseT = 1.0 - distNorm;',
    '    // 디졸브로 불투명 영역 조절 (안쪽에서만 불규칙하게)',
    '    float edgeVal = clamp(baseT + directedDn, 0.0, 1.0);',
    '',
    '    // edgeVal 클수록 불투명',
