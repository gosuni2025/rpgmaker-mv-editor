//=============================================================================
// OcclusionSilhouette.js - 캐릭터 가림 실루엣 효과
//=============================================================================
// 플레이어 캐릭터가 이미지 오브젝트(z=5) 뒤에 가려졌을 때,
// 가려진 부분을 실루엣으로 표시하여 플레이어 위치를 알 수 있게 합니다.
//
// 구현 원리:
// 1. 캐릭터 마스크: 플레이어/파티원만 별도 RenderTarget에 렌더
// 2. 오브젝트 마스크: 이미지 오브젝트만 별도 RenderTarget에 렌더
// 3. 합성 셰이더: 두 마스크가 겹치는 영역 = 가려진 부분 → 실루엣 효과
//=============================================================================
...
(function() {
    ...
    OcclusionSilhouette.config = {
        fillColor: [0.2, 0.4, 1.0],     // 내부 채움 색 (RGB 0~1)
        fillOpacity: 0.35,                // 내부 채움 투명도
        outlineColor: [1.0, 1.0, 1.0],   // 외곽선 색 (RGB 0~1)
        outlineOpacity: 0.8,              // 외곽선 투명도
        outlineWidth: 2.0,                // 외곽선 두께 (px)
        pattern: 'solid',                 // 채움 패턴: solid, empty, dot, diagonal, cross, hatch
        patternScale: 8.0,                // 패턴 크기 (px)
        includeFollowers: true            // 파티원도 포함
    };
