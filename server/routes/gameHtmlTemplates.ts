...
export function buildGameHtml(req: Request, res: Response, resolvedRuntimePath: string): void {
  ...
  const swScript = testSW ? `<script>
        (function() {
            ...
            navigator.serviceWorker.addEventListener('message', function(e) { var msg = e.data; if (!msg) return; if (msg.type === 'bundle-downloading') { var fd = document.getElementById('sw-files'); if (fd) { var sid = 'sw-f-'+msg.file.replace(/[^a-z0-9]/gi,'_'); if (!document.getElementById(sid)) { var row = document.createElement('div'); row.id = sid; row.style.cssText='display:flex;justify-content:space-between'; row.innerHTML='<span>'+msg.file+'</span><span id="'+sid+'-sz">...</span>'; fd.appendChild(row); } } } else if (msg.type === 'bundle-download-progress') { setBar(msg.totalSize > 0 ? msg.totalReceived/msg.totalSize*50 : 0); if (msg.files) msg.files.forEach(function(f) { var el = document.getElementById('sw-f-'+f.file.replace(/[^a-z0-9]/gi,'_')+'-sz'); if (el) el.textContent = f.total > 0 ? mb(f.received)+' / '+mb(f.total) : mb(f.received); }); } else if (msg.type === 'bundle-progress') { setTitle('[SW 번들 테스트] 압축 해제 중...'); setBar(50+(msg.totalFiles>0?msg.loadedFiles/msg.totalFiles*50:0)); var fd2=document.getElementById('sw-files'); if(fd2) fd2.textContent=msg.loadedFiles+' / '+msg.totalFiles+' 파일'; } else if (msg.type === 'bundle-ready' || msg.type === 'bundle-skip' || msg.type === 'bundle-error') loadMain(); });
